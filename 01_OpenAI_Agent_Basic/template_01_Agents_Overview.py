...
agent = Agent(
    name="Assistant",
    instructions="당신은 도움되는 도우미입니다.", # 모델의 페스소나 부여
...
result1 = await Runner.run(agent_no_output_type, input="서울의 날씨가 어때?")
#'서울의 현재 기온은 약 26.0°C예요.' '서울 현재 기온은 26.0°C예요.' :: 출력이 가변
...
print() ## Weather_Agent 를 최초 run  했지만 마지막에 handoff 되면서 last agent는 Weather_Agent 가 됨

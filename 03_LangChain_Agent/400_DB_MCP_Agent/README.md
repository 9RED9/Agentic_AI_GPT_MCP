...
필요한 패키지:
- `mcp` 또는 `fastmcp` - Model Context Protocol 및 FastMCP
- `langchain` - LangChain 프레임워크
- `langchain-mcp-adapters` - MCP-LangChain 어댑터
- `langchain-community` - LangChain 커뮤니티 유틸리티 (SQLDatabase 사용)
...
### 3. Streamlit UI 실행 (선택)

```bash
streamlit run streamlit_app.py
```

동일한 Chinook MCP 서버를 사용하는 채팅 UI입니다. `agent_client.py`의 Streamlit 버전이며 LangChain v1(`create_agent`)을 사용합니다.

### 4. 사용 예시

```
===== MCP MultiServer 클라이언트 초기화 중... =====
...
### MCP 서버 연결 실패
- `agent_server.py`가 별도 터미널에서 실행 중인지 확인
- 환경 변수가 올바르게 설정되었는지 확인
- `agent_client.py`의 경로(`./agent_server.py`)가 올바른지 확인
- stdio 통신이므로 포트 충돌은 없지만, 서브프로세스 실행 권한 확인
...
├── streamlit_app.py     # Chinook MCP Streamlit UI (agent_client의 Streamlit 버전)

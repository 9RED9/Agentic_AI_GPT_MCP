# notion_agent_client.py - 통합 클라이언트 (07_Notion_DB_MCP_Total)
# Chinook MCP + Notion MCP 서버를 하나의 Agent에 통합
# LangChain v1 (create_agent + MemorySaver)
        ...
        async with ClientSession(chinook_read, chinook_write) as chinook_session:
            ...
            async with stdio_client(notion_server_params) as (notion_read, notion_write):
                async with ClientSession(notion_read, notion_write) as notion_session:
                    ...
                    # ReAct Agent 생성 (LangChain v1 create_agent)
                    agent = create_agent(model=model, tools=all_tools, checkpointer=memory)
...
async def start_chatbot(agent):
    ...
    while True:
        ...
        # Agent의 최종 응답 출력 (content 없을 수 있음 대비)
        messages = response.get("messages", [])
        last = messages[-1] if messages else None
        content = getattr(last, "content", str(last)) if last else "(응답 없음)"
        print("응답:")
        print(content)
        print("\n" + "="*50 + "\n")
